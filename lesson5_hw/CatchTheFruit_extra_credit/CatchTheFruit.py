...
oScoreDisplay = pygwidgets.DisplayText(window, (10, WINDOW_HEIGHT - 200), '',
...
while True:
    ...
    for fruit in fruits:
        ...
        if basketRect.colliderect(fruitRect):
            ...
            # Increment the count for this fruit and reset its position
            fruit.incrementCount()
            score += points
            fruit.reset()
    ...
    # Monitor each fruit and add its count to the scoreboard text
    for fruit in fruits:
        fruit_type = fruit.fruitType.capitalize() + "s"
        caught_count = fruit.getCaughtCount()
        scoreboard_text += fruit_type + ": " + str(caught_count) + "\n"

    oScoreDisplay.setValue(scoreboard_text)
    ...
    # oFruit.draw()        # it tells each ball to draw itself

    for fruit in fruits:
        fruit.draw()       # it draws each fruit

    oRestartButton.draw()
    oBasket.draw()
    oDisplay.draw()
    oScoreDisplay.draw()    # it draws the scoreboard
